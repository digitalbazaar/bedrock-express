...
bedrock.events.on('bedrock.start', callbackify(async () => init()));

async function init() {
  const emit = promisify(bedrock.events.emit);
  // init
  await emit('bedrock-express.init', server);
  // basic config
  server.enable('trust proxy');
  server.disable('x-powered-by');
  // logger config
  const configLogger = await emit('bedrock-express.configure.logger', server);
  ...
  // static config
  const configStatic = await emit('bedrock-express.configure.static', server);
  ...
  // cache config
  const configCache = await emit('bedrock-express.configure.cache', server);
  ...
  const configBodyParser = await emit(
    'bedrock-express.configure.bodyParser', server);
  ...
  const configCookieParser = await emit(
    'bedrock-express.configure.cookieParser', server);
  if(configCookieParser !== false) {
    server.use(require('cookie-parser')(
      bedrock.config.express.session.secret));
  }
  // session config
  const configSession = await emit(
    'bedrock-express.configure.session', server);
  ...
  // router config
  await emit('bedrock-express.configure.router', server);
  // routes config
  await emit('bedrock-express.configure.routes', server);
  // error handler config
  await emit('bedrock-express.configure.errorHandlers', server);
  // unhandled error handler config
  const configUnhandledErrorHandler = await emit(
  ...
  // start
  await emit('bedrock-express.start', server);
  // ready check
  const cancel = await emit('bedrock-express.ready', server);
  // check if default attach canceled
  if(cancel !== false) {
