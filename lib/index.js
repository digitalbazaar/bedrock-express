...
bedrock.events.on('bedrock.start', async () => {
  ...
  // body parser config
  const {default: defaultOptions, routes} = config.express.bodyParser;
  // always apply route-specific configs (and apply first)
  for(const route in routes) {
    const routeOptions = routes[route];
    for(const key in routeOptions) {
      app.use(route, bodyParser[key]({...routeOptions[key]}));
    }
  }
  // emit event for app-specific body parser configuration
  const configBodyParser =
    await bedrock.events.emit('bedrock-express.configure.bodyParser', app);
  // apply default body parser config unless canceled
  if(configBodyParser !== false) {
    for(const key in defaultOptions) {
      app.use(bodyParser[key]({...defaultOptions[key]}));
    }
  }
...
function _handleJsonError() {
  return (err, req, res, next) => {
    ...
    } else if(res.statusCode === 413 || err.status === 413) {
      // entity too large
      res.statusCode = 413;
      err = new BedrockError('Content is too large.', {
        name: 'QuotaExceededError',
        cause: err,
        details: {httpStatusCode: 413, public: true}
      });
    } else {
 ...
 *
 * @param {object} app - The express app to update.
 */
