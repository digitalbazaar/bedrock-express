...
function init(callback) {
  async.auto({
    ...
    cookieParser: ['beforeCookieParser', (callback, results) => {
      if(results.beforeCookieParser === false) {
        return callback();
      }
...
function _routeConditionalMiddleware({fn/*, routeBlackList*/}) {
  // TODO: this would be passed in and would originate in the bedrock config
  const routeBlackList = new Set([
    '/consensus/continuity2017/voters/:voterId/gossip'
  ]);
  return function(req, res, next) {
    // iterate backwards because top level routes are defined near the end
    // TODO: is there some way to not have to iterate the stack on every
    // call?  Could the blackListed layers be stored somehow?
    for(let i = server._router.stack.length - 1; i >= 0; --i) {
      const layer = server._router.stack[i];
      if(!layer.route) {
        continue;
      }
      if(routeBlackList.has(layer.route.path) && req.path.match(layer.regexp)) {
        // blackList match do not execute the provided middeleware `fn`
        return next();
      }
    }
    fn(req, res, next);
